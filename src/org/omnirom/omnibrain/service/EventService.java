            ...
            try {
                ...
                switch (action) {
                    ...
                    case BluetoothA2dp.ACTION_CONNECTION_STATE_CHANGED:
                        connect = intent.getIntExtra(BluetoothProfile.EXTRA_STATE,
                                BluetoothProfile.STATE_CONNECTED) == BluetoothProfile.STATE_CONNECTED;
                        if (connect) {
                            MediaPlayerEvents.runEvent(context, getPrefs(context), "a2dp", mOverlayWidth);
                        }
                        break;
                    case AudioManager.ACTION_HEADSET_PLUG:
                        connect = intent.getIntExtra("state", 0) == 1;
                        if (connect) {
                            MediaPlayerEvents.runEvent(context, getPrefs(context), "headset", mOverlayWidth);
                        }
                        break;
                    case WifiManager.NETWORK_STATE_CHANGED_ACTION:
                        WifiEvents.runEvent(context, getPrefs(context));
                        break;
                }
            } finally {
                mWakeLock.release();

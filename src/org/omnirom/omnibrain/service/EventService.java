...
import org.omnirom.omnibrain.events.ByBTDeviceActions;
...
public class EventService extends Service {
    ...
    private ByBTDeviceActions mBTDeviceActions;
            ...
            try {
                ...
                switch (action) {
                    case BluetoothDevice.ACTION_ACL_CONNECTED:
                        if (DEBUG) Log.d(TAG, "BluetoothDevice.ACTION_ACL_CONNECTED");
                        device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
                        if (DEBUG) Log.d(TAG, "Bluetooth device: " + device.getName());
                        mMediaPlayerEvents.runEvent(getPrefs(context), "a2dp", mOverlayWidth, device.getName());
                        mBTDeviceActions.runConnectActions(context, getPrefs(context), device.getName());
                        break;
                    case BluetoothDevice.ACTION_ACL_DISCONNECTED:
                        if (DEBUG) Log.d(TAG, "BluetoothDevice.ACTION_ACL_DISCONNECTED");
                        device = intent.getParcelableExtra(BluetoothDevice.EXTRA_DEVICE);
                        if (DEBUG) Log.d(TAG, "Bluetooth device: " + device.getName());
                        mBTDeviceActions.runDisconnectActions(context, getPrefs(context), device.getName());
                        break;
                    case AudioManager.ACTION_HEADSET_PLUG:
                        if (intent.getIntExtra("state", 0) == 1) {
                            if (DEBUG) Log.d(TAG, "AudioManager.ACTION_HEADSET_PLUG");
                            mMediaPlayerEvents.runEvent(getPrefs(context), "headset", mOverlayWidth, null);
                        } else {
                            mMediaPlayerEvents.mLastUnplugEventTimestamp = System.currentTimeMillis();
                        }
                        break;
    ...
    public void onCreate() {
        ...
        mMediaPlayerEvents = new MediaPlayerEvents(this);
        mWifiEvents = new WifiEvents(this);
        mBTDeviceActions = new ByBTDeviceActions();
    }

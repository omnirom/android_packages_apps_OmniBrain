...
public class EventService extends Service {
    ...
    private void closeRunningApp(Context context) {
        if (runnigApp != null) {
            int mPersistentId = getRunningAppPersistentId(context);
            if (mPersistentId != -1) {
                killPackageProcess(mPersistentId, context);
            }
            runnigApp = null;
        }
    }

    private int getRunningAppPersistentId(Context context) {
        ...
        for (int i = 0; i < mTasks.size(); i++) {
            ...
            if (name.equals(runnigApp.getPackageName())) {
    ...
    private void killPackageProcess(int mId, Context context) {
        Context mContext = context;
        final ActivityManager am = (ActivityManager) mContext
                .getSystemService(Context.ACTIVITY_SERVICE);
        am.removeTask(mId);
